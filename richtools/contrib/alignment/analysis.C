...
void Analysis::Init(){
  ...
  for(int i=0;i<nSamples;i++){
    ...
    // Spatial ones
    alignmentParameters[0].push_back(alignmentParameters[ 0 ][0]+gRandom->Gaus()*0.2);
    alignmentParameters[1].push_back(alignmentParameters[ 1 ][0]+gRandom->Gaus()*0.2);
    alignmentParameters[2].push_back(alignmentParameters[ 2 ][0]+gRandom->Gaus()*0.6);
...
void Analysis::Loop(){
  ...
  while (event = chain->GetEvent()) {
    ...
    for(int i=0;i<nSamples;i++){
      ...
      // Convert the local parameters to the global ones
      AMSRotMat r; r.SetRotAngles(alignmentParameters[ 3 ][i],
				  alignmentParameters[ 4 ][i],
				  alignmentParameters[ 5 ][i]);

      AMSPoint nominal(0,0,RICHDB::RICradpos()-RICHDB::rad_height);
      AMSPoint shift; shift=nominal-(r*nominal);

      RichAlignment::Set(
			 alignmentParameters[ 0 ][i]+shift[0],
			 alignmentParameters[ 1 ][i]+shift[1],
			 alignmentParameters[ 2 ][i]+shift[2],
