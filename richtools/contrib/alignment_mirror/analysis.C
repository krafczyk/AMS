...
// DEFINE FOR USING NAF
//#define NAF

// DEFINE FOR FIXING THE ANGULAR PARAMETERS 
#define _FIX_ANGLES

// Define to correct the expansion length
#define REALDATA


// Define the step size (large step is OK for NaF)
#define LARGE_STEP

// If MAGIC STEP IS USED, TWO DIFFERENT SCALES ARE EXPLORED AT THE SAME TIME 
//#define MAGIC_STEP
...
void Analysis::Init(){
  ...
  for(int i=0;i<nSamples;i++){
    ...
    // Spatial ones
    alignmentParameters[0].push_back(alignmentParameters[ 0 ][0]+gRandom->Gaus()/3/2);
    alignmentParameters[1].push_back(alignmentParameters[ 1 ][0]+gRandom->Gaus()/3/2);
    alignmentParameters[2].push_back(alignmentParameters[ 2 ][0]+gRandom->Gaus());


#ifdef _FIX_ANGLES
    alignmentParameters[3].push_back(alignmentParameters[ 3 ][0]);
    alignmentParameters[4].push_back(alignmentParameters[ 4 ][0]);
    alignmentParameters[5].push_back(alignmentParameters[ 5 ][0]);
#else
    // Angular ones: adjusted
    
    alignmentParameters[3].push_back(alignmentParameters[ 3 ][0]+gRandom->Gaus()/1000*2);
    alignmentParameters[4].push_back(alignmentParameters[ 4 ][0]+gRandom->Gaus()/1000*4);
    alignmentParameters[5].push_back(alignmentParameters[ 5 ][0]+gRandom->Gaus()/1000*4);
    ...
    ////////////// AGL///////////////////
    // Spatial ones
    alignmentParameters[0].push_back(alignmentParameters[ 0 ][0]+gRandom->Gaus()*0.06);
    alignmentParameters[1].push_back(alignmentParameters[ 1 ][0]+gRandom->Gaus()*0.06);
    alignmentParameters[2].push_back(alignmentParameters[ 2 ][0]+gRandom->Gaus()*0.3);
...
#else
    // Angular ones: adjusted
    alignmentParameters[3].push_back(alignmentParameters[ 3 ][0]+gRandom->Gaus()/1000*0.7);
    alignmentParameters[4].push_back(alignmentParameters[ 4 ][0]+gRandom->Gaus()/1000*1.3);
    alignmentParameters[5].push_back(alignmentParameters[ 5 ][0]+gRandom->Gaus()/1000*1.3);
#endif

#endif

    /////////////////////////////////////////////////////////////// MAGI STEP NOT DEFINED
#else

    // Spatial ones
    alignmentParameters[0].push_back(alignmentParameters[ 0 ][0]+gRandom->Gaus()*0);//0.2);
    alignmentParameters[1].push_back(alignmentParameters[ 1 ][0]+gRandom->Gaus()*0);//0.2);
    alignmentParameters[2].push_back(alignmentParameters[ 2 ][0]+gRandom->Gaus()*0.6);//0.6);


    // Angular ones: adjusted
    
    alignmentParameters[3].push_back(alignmentParameters[ 3 ][0]+gRandom->Gaus()*3e-3);
    alignmentParameters[4].push_back(alignmentParameters[ 4 ][0]+gRandom->Gaus()*3e-3);
    alignmentParameters[5].push_back(alignmentParameters[ 5 ][0]+gRandom->Gaus()*3e-3);
...
void Analysis::Loop(){
  ...
  while (event = chain->GetEvent()) {
    if(gRandom->Uniform()<0.3) continue;
    ...
    for(int i=0;i<nSamples;i++){
      ...
      if(i==0){
	for(int jj=0;jj<ring->_beta_reflected.size();jj++)
	  testigo.Fill(ring->_beta_reflected[jj]);
      }
	...
	first=0;
      ...
      int tile_number=0;
      entries[tile_number]++;
      for(int j=0;j<ring->_beta_reflected.size();j++){
	betaHitHistograms[tile_number][i].Fill(ring->_beta_reflected[j]);
	betaHitSummary[i].Fill(ring->_beta_reflected[j]);
      }
...
TF1 *Fita(TH1F *proyection,double &mean,double &mean_error,double &sigma,double &sigma_error){
  ...
  proyection->Fit("pol0","","",0.98,1.02);
  Double_t ref=proyection->GetFunction("pol0")->GetParameter(0);
...
int main(int argc,char **argv){
  ...
  */
  ...
  for(int i=0;i<nSamples;i++){
    ...
    if(i==0) continue; // skip the original one
    ...
    if(sum2/total<best){
      best=sum2/total;
      best_i=i;
      analysis.testigo=analysis.betaHitHistograms[0][i];
    }
  }
#ifdef DEBUG
  fclose(file_samples);
