...
c--------------------------------- sample u=x+y
      ...
      if(cc.lt.0.) cc=0.
13    continue
      c=Exp(RNDMDPM(2.)*Log((umax+Sqrt(cc))/umin))
      uu=umin*(c**2+1.)/2./c
...
C        IF ( R*W.LT.RM(1)*RNDMDPM(AI) ) GOTO 10
        IF(WIK.LT.RNDMDPM(AI)) GOTO 10
...
20      CALL HAX1X2
        ...
        IF ( R*W.LT.RM(M)*RNDMDPM(AI) ) GOTO 20
        IF ( RNDMDPM(AJ).LE.0.5D0 ) V = U
      ELSEIF ( M.EQ.3 ) THEN
30      CALL HAX1X2
        V  =-0.5*W1/(W1+RNDMDPM(AI)*W)
        ...
        WIK=R*W/RMAX
        IF(WIK.GT.1.D0) WRITE(6,*) ' HARKIN : WIK > 1 : ',M,R
C        IF ( R*W.LT.RM(3)*RNDMDPM(AI) ) GOTO 30
        IF(WIK.LT.RNDMDPM(AI)) GOTO 30
      ELSEIF ( M.EQ.5 ) THEN
50      CALL HAX1X2
        ...
        WIK=R*W/RMAX
        IF(WIK.GT.1.D0) WRITE(6,*) ' HARKIN : WIK > 1 : ',M,R
C        IF ( R*W.LT.RM(5)*RNDMDPM(AI) ) GOTO 50
        IF(WIK.LT.RNDMDPM(AI)) GOTO 50
      ELSEIF ( M.EQ.6 ) THEN
60      CALL HAX1X2
        V  =-0.5*(1.+W)+RNDMDPM(AI)*W
        U  =-1.-V
        R  = (4./9.)*(U*U+V*V)*AXX
        IF ( R*W.LT.RM(6)*RNDMDPM(AI) ) GOTO 60
      ELSEIF ( M.EQ.7 ) THEN
70      CALL HAX1X2
        V  =-0.5*W1/(W1+RNDMDPM(AI)*W)
        U  =-1.-V
        ...
        WIK=R*W/RMAX
        IF(WIK.GT.1.D0) WRITE(6,*) ' HARKIN : WIK > 1 : ',M,R
C        IF ( R*W.LT.RM(7)*RNDMDPM(AI) ) GOTO 70
        IF(WIK.LT.RNDMDPM(AI)) GOTO 70
        IF ( RNDMDPM(AJ).LE.0.5D0 ) V = U
      ELSEIF ( M.EQ.8 ) THEN
80      CALL HAX1X2
        V  =-0.5*AXX/(W1+2.*RNDMDPM(AI)*W)
        ...
        WIK=R*W/RMAX
        IF(WIK.GT.1.D0) WRITE(6,*) ' HARKIN : WIK > 1 : ',M,R
C        IF ( R*W.LT.RM(8)*RNDMDPM(AI) ) GOTO 80
        IF(WIK.LT.RNDMDPM(AI)) GOTO 80
      ELSEIF ( M.EQ.-1 ) THEN
90      CALL HAX1X2
        WL = LOG(W1)
        V  =-EXP(-0.6931472+RNDMDPM(AI)*WL)
        ...
        IF ( R*W.LT.RM(-1)*RNDMDPM(AI) ) GOTO 90
      ENDIF
