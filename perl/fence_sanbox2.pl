...
#$FILE="<./cluster.conf";
    ...
    close(FILE);
    if(not defined $ipaddr or not defined $password or not defined $login or not defined $port){
        die "not defined ip pas log port $ipaddr $password $login $port \n";
    }
    unshift @ARGV,"-n$port"; 
    unshift @ARGV,"-a$ipaddr"; 
    unshift @ARGV,"-l$login"; 
    unshift @ARGV,"-oenable"; 
    unshift @ARGV,"-p$password"; 
    unshift @ARGV,"-q"; 
