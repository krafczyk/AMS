...
if($ok){
    ...
    my $sql = "select count(run) from amsdes.ntuples where  path like '%ISS.B700P/$ds%' and path not like '/castor%' $run2p";
         my $ret=$o->Query($sql);
    if($ret->[0][0]>0){
        print "Not castor file exists $ret->[0][0] \n";
 $sql = "select count(run) from amsdes.ntuples where  path like '%ISS.B700P/$ds%' and path not like '/castor%' and castortime=0 $run2p";
       $ret=$o->Query($sql);
  if($ret->[0][0]>0){
    ...
    if($eos){
 $sql = "select count(run) from amsdes.ntuples where  path like '%ISS.B700P/$ds%' and eostime=0 $run2p";
       $ret=$o->Query($sql);
  ...
  if($ret->[0][0]>0){
        ...
        if($eos>1){
            ...
            my $castorsuffix="?svcClass=amscdr";
             $sql = "select path from amsdes.ntuples where  path like '%ISS.B700P/$ds%' and eostime=0 $run2p";
       $ret=$o->Query($sql);
    foreach my $file (@{$ret}){
...
seteos:
 ...
 $sql = "select count(path) from amsdes.ntuples where  path like '%ISS.B700/$ds%' and run in (select run from   ntuples   where  path like '%ISS.B700P/$ds%' and path  like '/castor%') $run2p";
       $ret=$o->Query($sql);
  if($ret->[0][0]>0){
...
#        exit;
    ...
    $sql="select path,jid,eostime from ntuples   where  path like '%ISS.B700P/$ds%' and path  like '/castor%' $run2p";
         $ret=$o->Query($sql);
    foreach my $file (@{$ret}){
        my $line=$file->[0];
        my $eos_filein=$line;
        ...
        if($file->[2]>0){
          ...
          $eos_fileout=~s/ISS.B700P/ISS.B700/;
