# $Id: RemoteClient.pm,v 1.341 2005/10/28 19:48:40 choutko Exp $
#
    ...
    if( defined $r3->[0][0]){
     foreach my $job (@{$r3}){
       ...
       if ($tmoutflag == 1) {
         ...
         if ($update == 1) {
          $sql=" update jobs set timekill=$timenow+3600*24+1, timeout=$timenow+3600*24*7-$job->[1]+1 where jid=$jid";
          $self->{sqlserver}->Update($sql); 
