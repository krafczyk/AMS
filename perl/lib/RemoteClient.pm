...
jpid=productionset.did and realtriggers>0 and cid=".$self->{CCID}." and productionset.did in (select did from productionset where status='Active')");
	if ( defined $ret->[0][0] ) {
        ...
        if($completed_jobs > 0){
	    if($max_jobs < $capacity_jobs-$total_jobs+$completed_jobs){
		$max_jobs = $capacity_jobs-$total_jobs+$completed_jobs;
            }
        }
          }
        }
...
# DST output format
            ...
            print "<tr><td><font size=\"-1\"<b>\n";
            print "<tr><td><font size=\"-1\"<b>\n";
            print "<INPUT TYPE=\"radio\" NAME=\"RootNtuple\" VALUE=\"1=0 168=9000000 170=$self->{Build} 127=2 128=\" $defROOT><b> RootFile </b><BR>\n";
