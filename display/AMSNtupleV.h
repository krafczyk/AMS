//  $Id: AMSNtupleV.h,v 1.23 2008/12/18 21:57:40 pzuccon Exp $
  ...
  TrRecHitV(AMSEventR *ev,int ref,int mult=0):AMSDrawI(ev,ref),TMarker3DCl(){
    ...
    if(pcl){
      SetSize(pcl->GetECoord()[0]<0.5?pcl->GetECoord()[0]*100:pcl->GetECoord()[0],
	      pcl->GetECoord()[1]*100,
	      (sqrt(pcl->Sum()/10)<6)?sqrt(pcl->Sum()/10):8);
	...
	if(pcl->GetYCluster()){
	  ...
	  for (int k=0;k<kmax;k++)x[k]=pcl->GetYCluster()->GetSignal(k)/
	    (pcl->GetYCluster()->GetTotSignal()+1.e-20);
	  SetProfileY(pcl->GetYCluster()->GetLength(),x);
	  SetShowProfileY(true);
	}
	if(pcl->GetXCluster()){
	  int kmax=pcl->GetXCluster()->GetLength();
	  if(kmax>sizeof(x)/sizeof(x[0]))kmax=sizeof(x)/sizeof(x[0]);
	  for (int k=0;k<kmax;k++)x[k]=pcl->GetXCluster()->GetSignal(k)/
...
public:
  ...
  TrMCClusterV(AMSEventR *ev,int ref):AMSDrawI(ev,ref),TMarker3DCl(){
    ...
    if(pcl){
      SetPosition(pcl->_xgl[0],pcl->_xgl[1],pcl->_xgl[2]);
