...
// Check for the list
	  ...
	  strcpy(err_mess, "Cannot open the list in oocUpdate mode"); 
        ...
        if(!listH -> AddEvent(run, eventNumber, eventID, eventH, eventW)) {
	 strcpy(err_mess, "Cannot add the event to the list"); goto error; }

        if (eventH == NULL) {strcpy(err_mess, "eventH == NULL");
        goto error; }
         }
        }
error:
                             ...
                             integer flag, ooHandle(AMSEventList)& listH)
{
	ooStatus	rstatus = oocError;	// Return status
        ...
        // Create the list, it doesn't exist
        _databaseH = _session -> DefaultDatabase();
        ...
        if (flag != -1 ) {
          ...
          if (mapsH.exist(_databaseH, "Maps", oocUpdate)) {
            eventMapH = new (mapsH) ooMap;
            rstatus = eventMapH.nameObj(mapsH,listName);
            if (rstatus != oocSuccess) {
              cerr<<"LMS::AddList-E- error creating map "<<listName<<endl;
            }
          }
        }
...
//-
{
	ooStatus	       rstatus;             // Return status
        ...
        if (ReadStartEnd == 1 || ReadStartEnd == -2) {
         rstatus = Start(mode);
         if (rstatus != oocSuccess) return rstatus;
         cout <<"LMS::GetNEvent: -I- StartTransaction"<<endl;
        }
        ...
        if (rstatus == oocSuccess) {
         if (eventN < 0) {
          if (eventID) {
           rstatus = listH -> FindEvent(eventID, mode, eventH);
           if (rstatus != oocSuccess) {
            cout <<"LMS::GetNEvent: error in FindEvent "<<endl;
            do_abort = 1;
            goto error;
           }
           position = eventH -> getPosition();
          ...
          position = eventN;
          if (position < 1 ) {
           cout<<"LMS::GetNmceventg -W- old position "<<position
               <<" will be set to 1"<<endl;
           position = 1;
          }
         }
         rstatus = oocError;
         ...
         if (nevents != 0) {
          if (run > 0)
            (void) sprintf(pred,"_Position=%d && _run=%d",position,run);
          if (run < 1)
            (void) sprintf(pred,"_Position=%d && _run>%d",position,run);
          eventItr.scan(listH, mode, oocAll, pred);
          cout<<"LMS::GetNmceventg-I- scan event of "<<pred<<endl;
          if (eventItr.next() != NULL) 
            rstatus = listH -> CopyMCeventg(eventID, eventItr, mode);
         } else {
          do_abort = 1;
        }
error:
