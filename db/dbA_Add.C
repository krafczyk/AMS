//  $Id: dbA_Add.C,v 1.10 2001/01/22 17:32:28 choutko Exp $
...
ooStatus LMS::AddEvent(integer run, integer eventNumber, time_t time, 
...
//    if (Mode() != oocUpdate) {
      if (mode   != oocUpdate) {
     Error("AddEvent -E- Cannot add event in non-oocUpdate mode");
     return rstatus;
    }
...
//
      ...
      if( rawevents(mode) ) {
       ...
       if (raweventItr != NULL) {
        raweventH = raweventItr;
        cout <<"overwrite Raw ";raweventH -> print();
        ooDelete(raweventH);
        rawCont() -> decNEvents();
       }
      }
//
   ...
   if (rawevents(mode) ) {
     rstatus = AddRawEvent(tageventH, run, eventNumber, time, runtype,
                           WriteStartEnd);
     if (rstatus != oocSuccess) Fatal("Cannot add Raw event");
     rawCont() -> incNEvents();
    }
...
ooStatus LMS::ReadEvents(integer& run, integer& eventNumber, 
                         integer   nevents,
                         time_t& time, 
                         integer StartCommit)
//
...
// one new argument called uinteger usec is addedd !!!!!!!!!!!!!!
     ...
     eventH = eventItr;
     if(recoevents(mode)) { // read full reconstructed event from dbase
      dbH = recodb();
