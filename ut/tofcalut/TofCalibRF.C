...
Int_t TofCalib::fittd(){//--->Tdelv-calib: get the slope,td0,chi2
  ...
  Double_t t0d[34]={
                   ...
                   -13.98, -13.18, -12.30, -10.86, -11.89, -11.82, -11.24, -10.47, 
                    -9.00,  -8.27,  -9.70, -10.25, -11.26, -10.79, -11.46, -11.67, -13.23, -12.99, 
...
//
  for(i=0;i<kNtofl-1;i++){//loop over pairs Lr->Lk, k=(2),3,4
    ...
    if(il>(refl-1)){//skip missing pair Lr->L2 if refl=2
      for(ib=0;ib<kNtofb[il];ib++)f2[i]+=s6[i][ib]*x[1+barseqn(il,ib)];//l=(2),3,4
    }
  }
//
  for(ib=0;ib<kNtofb[refl-1];ib++)f3+=s7[ib]*x[1+barseqn(refl-1,ib)];//l=r
//
  if(refl==1){// f6=0(skipped) if refl=2
    for(i=0;i<kNtofb[refl-1];i++){
      for(j=0;j<kNtofb[1];j++)f6+=s12[i][j]*x[1+barseqn(refl-1,i)]*x[1+barseqn(1,j)];//Lr*(2)
    }
  }
//
  for(i=0;i<kNtofb[refl-1];i++){//loop over bars in refl
    for(j=0;j<kNtofb[2];j++)f7+=s13[i][j]*x[1+barseqn(refl-1,i)]*x[1+barseqn(2,j)];//Lr*3
  }
//
  for(i=0;i<kNtofb[refl-1];i++){
    for(j=0;j<kNtofb[3];j++)f8+=s14[i][j]*x[1+barseqn(refl-1,i)]*x[1+barseqn(3,j)];//Lr*4
  }
//
  for(i=0;i<kNtofl-1;i++){ 
    f9[i]=0.;
    il=i+1;
    if(il>(refl-1)){//skip missing pair Lr->L2 if refl=2
      for(ib=0;ib<kNtofb[il];ib++)f9[i]+=s15[i][ib]*x[1+barseqn(il,ib)];//l=(2),3,4
    }
  }
//
  for(ib=0;ib<kNtofb[refl-1];ib++)f10+=s16[ib]*x[1+barseqn(refl-1,ib)];//l=r
//
  f=s1
   +f1[0]+f1[1]+f1[2]+f1[3]
   +s4*x[0]*x[0]
...
Int_t TofCalib::fittz(){  // Tzslw-calibr. fit procedure, f.results->slope,tzero[][]->file
  Int_t i,id,ii,j,ier,il,ib,npar(kTofNBars+2),nparr,seqnum;
  Int_t cstatus(1);//bad
//
  Char_t pnm[6];
  Char_t p1nam[6];
...
//  gMinuit->SetPrintLevel(0);
  ...
  for(i=0;i<npar;i++){//<--- preset/fix parameters
    strcpy(pnm,pnam[i]);
    ier=0;
    gMinuit->mnparm(i,pnm,start[i],step[i],plow[i],phigh[i],ier);
    if(ier!=0){
