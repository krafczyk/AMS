...
$usetrd_bt=$set_fram->Checkbutton(-text=>"UseTRD", -font=>$font2,
                                        -indicator=>0,
                                        -borderwidth=>3,-relief=>'raised',
				        -selectcolor=>orange,-activeforeground=>red,
				        -activebackground=>yellow, 
...
#
  $cmd="ls -ogt --time-style=long-iso ".$dir."/TofTAUC*.*[0-9]".".log.*[0-9] |";
  @array=();
...
#
  ...
  if($sdtmdfcal!=1 && $tm0slwcal!=1 && $amnormcal!=1){
    show_warn("   <--- Above selection was not done properly !!!, repeate it...");
    return;
  }
#
...
#
  ...
  if($magstext eq "MagnetON"){# trk+trd usage mode
    ...
    $jpar6=0;# trk
    if($usetrd==1){$jpar6=-1;}# trd when no trk
  }
  if($magstext eq "MagnetON"){$jpar7=1;}
  else{$jpar7=-1;}
  $jpar8=$ncpus;
  $jpar9=$Evs2Read;
#---
...
#---
  ...
  my $dir=$workdir.$amsjwd;
  my $logf=$dir."/TofTAUC".$CalCode.".".$CalRun1.".log.%J";
  my $logef=$dir."/TofTAUC".$CalCode.".".$CalRun1.".loge.%J";
  my $comm2run;
  my $fn;
  ...
  for($lf=0;$lf<$nellfl;$lf++){#<---logf-loop (tempor.created above)
    ...
    for($kind=0;$kind<$KilledJobs;$kind++){#<-- look into killed jobs list
      ...
      if($kjpid==$lfpid){#<-- delete related log(e)-files
        $fn="TofTAUC".$ccode.".".$strun.".log.".$lfpid;
...
#      
        $fne="TofTAUC".$ccode.".".$strun.".loge.".$lfpid;
...
#    return;
  ...
  else{show_messg("\n   <--- JobOutputControl: Found $FiniJobCount finished jobs !");}
#
...
#-> search/read "results"-block:
	...
	if((($ccode%100)/10)>0){#<--Tzslw-results check
	  @calqual=split(/\s+/,$lfresult[1]);
	  if($calqual[0]>0.8 && $calqual[1]<0.5){#<-- Tzslw ok(good.stat.counters% | resol1(2layers diff(calc/ref) aver rms))
	    $ccodeok+=10;
	  }
	  else{show_messg("\n   <--- Bad Tzslw-results: $lfresult[1]\n");}
	}
	if(($ccode%10)>0){#<--Amplf-results check
	  @calqual=split(/\s+/,$lfresult[2]);
	  if($calqual[0]>0.8 && $calqual[1]>0.7 && $calqual[2]>0.1){#<-- Amplf pars ok:
#                          rel.gains(good.stat.sides%) | absnorm(dood.stat.refcounters%) | a2d-ratio(good.stat.ch%)                 
	    $ccodeok+=1;
	  }
	  else{show_messg("\n   <--- Bad Amplf-results: $lfresult[2]\n");}
	}
      }
      else{
...
#    $logf_lbx->selectionClear($index);
    $line=$LogFileList[$index];
    ($lfpid,$ccode,$strun,$time,$utcmin,$stat)=unpack($LogfListFmt,$line);
    $fn="TofTAUC".$ccode.".".$strun.".loge.".$lfpid;
    $fname=$workdir.$amsjwd."/".$fn;
    $stat=system("nedit -read $fname");
    if($stat != 0){
