...
c        close(20)
        ...
        write(*,*)cc, ' trigger combinations found'
         ...
         call hbook1(1974,'mass',200,0.,3.,0.)
         call hbook2(21973,'mass vs m',100,0.,4.,100,0.,6.,0.)
         ...
         call hbook1(3111,'perrmass',200,0.,1.,0.)
         call hbook2(3112,'perrmass vs mass',100,0.,1.,100,0.,3.,0.)
...
c                   cuts(8)=.true.
     ...
     +              betachi2(pbetap(1)).lt.5..and.r.lt.1.5
*
* Cerenkov cuts
*
                    xll=-57.
     ...
     +              cooctc(2,1,1).lt.ylr
                    ...
                    cuts(9)=(cuts(9).and.(xx.lt.1..and.xx1.eq.0)).and.
     +              (beta(pbetap(1))).gt.0
                    cuts(9)=cuts(9).and.betapattern(pbetap(1)).eq.0
     +              .and.pmass(1).gt.1.e-10
                    if(cuts(9))then
                       call hf1(3111,perrmass(1)/pmass(1),1.)
                       call hf2(3112,perrmass(1),pmass(1),1.)
                    endif
                    cuts(9)=cuts(9).and.pmass(1).gt.0.and.
     +              perrmass(1).lt.0.4*pmass(1).and.perrmass(1).lt.0.25
                    if(cuts(9))then
                    do i=1,ntrclmc
     ...
     +                  (gp0(3,iptr)-hp0(3,2,iptr))**2
                    ...
                    r=chi2fastfit(iptr)-hchi2(1,iptr)-hchi2(2,iptr)
                    if(pmass(1).gt.0)then 
                      call hf1(1973,pmass(1),1.)
                      call hf2(21973,pmass(1),abs(pmom(1)),1.)
                      if(abs(pmom(1)).gt.0.5)
     +                 call hf1(1974,pmass(1),1.)
                    endif
                            cy=cos(yaws)
                            sy=sin(yaws)
                            cr=cos(rolls)
