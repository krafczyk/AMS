...
VZERO(_nhits,sizeof(_nhits)/sizeof(integer));
 ...
 integer TriggerLVL3::TOFOr(uinteger paddle,uinteger plane){
         return plane< TOF2GC::SCLRS && paddle<TOF2GC::SCBRS[plane] ? _TOFOr[plane][paddle]:-1;}
//
 integer TriggerLVL3::TOFInFastTrigger(uinteger paddle, uinteger plane){
       return plane< TOF2GC::SCLRS && paddle<TOF2GC::SCBRS[plane] ? !_TOFStatus[plane][paddle]:-1;}
...
void TriggerLVL3::init(){
 ...
 integer ltop,lbot;
  ...
  if(LVL3FFKEY.UseTightTOF){
    ...
    for(i=0;i<TOF2GC::SCBRS[ltop];i++){
      for(j=0;j<TOF2GC::SCBRS[lbot];j++)iftxt>>_TOFPattern[j][i];
    }
    for(i=0;i<TOF2GC::SCLRS;i++){
      for(j=0;j<TOF2GC::SCBRS[i];j++)iftxt>>_TOFOr[i][j];
    }
    for(i=0;i<TOF2GC::SCLRS;i++){
      for(j=0;j<TOF2GC::SCBRS[i];j++)iftxt>>_TOFStatus[i][j];
    }
    if(iftxt.eof() ){
    ...
    for(i=0;i<TOF2GC::SCBRS[ltop];i++){
      for(j=0;j<TOF2GC::SCBRS[lbot];j++)_TOFPattern[j][i]=1;
    }
    for(j=0;j<TOF2GC::SCLRS;j++){
      for(i=0;i<TOF2GC::SCBRS[j];i++)_TOFOr[j][i]=1;
    }
  }

    for(j=0;j<TOF2GC::SCLRS;j++){
      for(i=0;i<TOF2GC::SCBRS[j];i++){
...
*/
    ...
    if(oftxt){
      ...
      for(i=0;i<TOF2GC::SCBRS[ltop];i++){
        oftxt <<"i "<<i<<" ";
        for(j=0;j<TOF2GC::SCBRS[lbot];j++)oftxt <<_TOFPattern[j][i]<<" ";
        oftxt <<endl;
      }    
      oftxt<<endl;
      oftxt<<endl;
      ...
      for(i=0;i<TOF2GC::SCLRS;i++){
        oftxt <<"i "<<i<<" ";
        for(j=0;j<TOF2GC::SCBRS[i];j++)oftxt <<_TOFOr[i][j]<<" ";
        oftxt <<endl;
      }    
      oftxt<<endl;
      ...
      for(i=0;i<TOF2GC::SCLRS;i++){
        oftxt <<"i "<<i<<" ";
        for(j=0;j<TOF2GC::SCBRS[i];j++)oftxt <<_TOFStatus[i][j]<<" ";
        oftxt <<endl;
      }    
      oftxt<<endl;
...
void TriggerLVL3::addtof(int16 plane, int16 paddle){
  if(plane >=0 && plane < TOF2GC::SCLRS && paddle>=0 && paddle <TOF2GC::SCBRS[plane] && 
...
//----------------------------------------------------------------
  integer TriggerLVL3::tofok(){ //Vitali mustcheck
    ...
    int ntof=0;
      for(i=0;i<_NTOF[0];i++){
        for(j=0;j<_NTOF[TOF2GC::SCLRS-1];j++){
          if(_TOFPattern[_TOFAux[0][i]][_TOFAux[TOF2GC::SCLRS-1][j]]){
              goto out;
          }
        }
      }
...
out:
    for (i=0;i<TOF2GC::SCLRS;i++){
      if((_NTOF[i]> 2) || (_NTOF[i] == 0 && (i == 0 || i==TOF2GC::SCLRS-1)))return 0;
...
//---------------------------------------------------------------
  ...
  void TriggerLVL3::preparetracker(){
   ...
   cooup=cooup/_NTOF[0];   
   for(i=0;i<_NTOF[TOF2GC::SCLRS-1];i++)coodown+=_TOFCoo[TOF2GC::SCLRS-1][_TOFAux[TOF2GC::SCLRS-1][i]][1];
   coodown=coodown/_NTOF[TOF2GC::SCLRS-1];   
   for(i=0;i<TKDBc::nlay();i++){
     geant coo=coodown+(cooup-coodown)/(_TOFCoo[0][0][2]-_TOFCoo[TOF2GC::SCLRS-1][0][2])*
     (_TrackerCooZ[i]-_TOFCoo[TOF2GC::SCLRS-1][0][2]);
