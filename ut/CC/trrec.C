...
integer AMSTrTrack::buildPathIntegral_old(integer refit){
  ...
  while (1) {
	...
	AMSTrTrack ptest;
      ...
      for (int pat=0;pat<TKDBc::npat();pat++){
            ...
            // Skip pattern if any layer has too many points on it
            for (int i=0; i<=fp; i++) {
              int il = TKDBc::patconf(pat,i)-1;
              if (nelem[il]>TRFITFFKEY.MaxTrRecHitsPerLayer) goto next_pattern;
            }
            ...
            for (phit[0]=AMSTrRecHit::firstgood(pat,0);
               ...
               for (phit[fp]=AMSTrRecHit::firstgood(pat,fp);
                  ...
                  for (int ilay=1;ilay<fp;ilay++){
                        if (AMSTrRecHit::firstgood(pat,ilay)==NULL) {
                              goto next_pattern;
                        }
                  }
...
integer AMSTrTrack::buildWeakPathIntegral_old(integer refit){
  {
    int nrh=0;
    for(int i=0;i<TKDBc::nlay();i++){
     nrh+= (AMSEvent::gethead()->getC("AMSTrRecHit",i))->getnelem();
...
integer AMSTrTrack::buildFalseXPathIntegral(integer nptmin){
  {
    int nrh=0;
    for(int i=0;i<TKDBc::nlay();i++){
     nrh+= (AMSEvent::gethead()->getC("AMSTrRecHit",i))->getnelem();
  ...
  for (int i=0; i<trconst::maxlay; i++) { 
     nelem[i] = (AMSEvent::gethead()->getC("AMSTrRecHit",i))->getnelem();
#ifdef __AMSDEBUG__
     printf("%d points on layer %d\n",nelem[i],i);
#endif
  }
 
  while (1) {

      if (NTrackFound+NTrackFalseX>=Vtxconst::maxtr) break;
	...
	AMSTrTrack ptest;
      ...
      for (int pat=0;pat<TKDBc::npat();pat++){
            if (TKDBc::patpoints(pat)>nptmin) continue;
            if (!TKDBc::patallowFalseX(pat)) continue;
            ...
            // Skip pattern if any layer has zero or too many points on it
            for (int i=0; i<=fp; i++) {
              if (AMSTrRecHit::firstgood_FalseX(pat,i)==NULL) goto next_pattern;
              int il = TKDBc::patconf(pat,i)-1;
              if (nelem[il]>TRFITFFKEY.MaxTrRecHitsPerLayer) goto next_pattern;
            }
                  ...
                  for (phit1=AMSTrRecHit::firstgood_FalseX(pat,fp-1);
	                  ...
	                  if (ptest._Chi2WithoutMS>TRFITFFKEY.Chi2WithoutMS) continue;
                        ...
                        for (int j=fp-3; j>=0; j--) {
                          ...
                          AMSTrRecHit* pbest = NULL;
                          double chi2tmp = TRFITFFKEY.Chi2WithoutMS;
                          for (phitl=AMSTrRecHit::firstgood_FalseX(pat,j);
                                   phitl; phitl=phitl->nextgood_FalseX_path(par)){
                              ptest._Pthit[0] = phitl;
                              ptest.SimpleFit(hit_err);
	                        if (ptest._Chi2WithoutMS<chi2tmp) {
                                    chi2tmp = ptest._Chi2WithoutMS;
                          ...
                          if (_NoMoreTime()) { 
                              remove_track(ptrack); 
                              if (AMSEvent::debug){
                                cout << " buildFalseXPathIntegral Cpulimit Exceeded!!!! " << endl;
                              }
                              throw AMSTrTrackError(" Cpulimit Exceeded ");
                              return NTrackFalseX;
	                    }
...
next_pattern:
      ...
      if (ptrack){ 
          ...
          // Fit track; add it to the container list
          ptrack->Fit(0);
          int iflag_falsex = AMSTrTrack::_addnextFalseX(pat, nhits, phit);
          if (iflag_falsex>0) {
            NTrackFalseX++;
            remove_track(ptrack);
          } else if (iflag_falsex<0) {
            NTrackFound++;
            if(AMSEvent::debug){
            ...
            // Get out if we were not succesful (but why?)
            remove_track(ptrack); 
            return NTrackFalseX;
          }
      } else {
...
integer AMSTrTrack::buildFalseXPathIntegral_old(integer nptmin){
  {
    int nrh=0;
    for(int i=0;i<TKDBc::nlay();i++){
     nrh+= (AMSEvent::gethead()->getC("AMSTrRecHit",i))->getnelem();
  ...
  while (1) {
	...
	AMSTrTrack ptest;
      ...
      for (int pat=0;pat<TKDBc::npat();pat++){
            ...
            ptest._NHits = fp+1;
            ptest._Pattern = pat;
            for (phit[0]=AMSTrRecHit::firstgood_FalseX(pat,0);
                              phit[0]; phit[0]=phit[0]->nextgood_FalseX()){
               ptest._Pthit[0] = phit[0];
               AMSPoint x0 = phit[0]->getHit();
               for (phit[fp]=AMSTrRecHit::firstgood_FalseX(pat,fp);
                              phit[fp]; phit[fp]=phit[fp]->nextgood_FalseX()){

                  // Skip whole pattern if not hits in the relevant layers
                  for (int ilay=1;ilay<fp;ilay++){
                        if (AMSTrRecHit::firstgood_FalseX(pat,ilay)==NULL) {
                              goto next_pattern;
                        }
                  }
	            ...
	            number par[2][3];
                  ...
                  // Initial hit choice on intermediate layers
                  if (NTrackFound<0) NTrackFound = 0;
                  if (NTrackFalseX<0) NTrackFalseX = 0;
                  for (int ilay=1;ilay<fp;ilay++){
                        phit[ilay]=AMSTrRecHit::firstgood_FalseX_path(pat,ilay,par);
                        if (phit[ilay]==NULL) goto next_fp;
                        ptest._Pthit[ilay] = phit[ilay];
                  }
...
#ifdef __AMSDEBUG__
                              printf("indexes :");
                              for (int i=0; i<ptest.getnhits(); i++) {
                                    printf(" Lay %d Hit %x",ptest.getphit(i)->getLayer()-1,ptest.getphit(i));
                              }
                              printf("\n");
                              ptest._printEl(cout);
#endif
*/
                        ...
                        // Break when no more intermediate hit combinations
                        if ( !ptest.next_combination_FalseX(1,fp-1,par) ) {
                              goto next_fp;
                        }
  ...
  while (1) {
	...
	AMSTrTrack ptest;
      ...
      for (int pat=0;pat<TKDBc::npat();pat++){
            ...
            // Skip pattern if any layer has zero or too many points on it
            for (int i=0; i<=fp; i++) {
              if (AMSTrRecHit::firstgood_FalseTOFX(pat,i)==NULL) goto next_pattern;
              int il = TKDBc::patconf(pat,i)-1;
              if (nelem[il]>TRFITFFKEY.MaxTrRecHitsPerLayer) goto next_pattern;
            }
                  ...
                  for (phit1=AMSTrRecHit::firstgood_FalseTOFX(pat,fp-2);
	                  ...
	                  if (ptest._Chi2WithoutMS>TRFITFFKEY.Chi2WithoutMS) continue;
                        ...
                        for (int j=fp-3; j>=0; j--) {
                          ...
                          AMSTrRecHit* pbest = NULL;
                          double chi2tmp = TRFITFFKEY.Chi2WithoutMS;
                          for (phitl=AMSTrRecHit::firstgood_FalseTOFX(pat,j);
                                   phitl; phitl=phitl->nextgood_FalseTOFX_path(par)){
                              ptest._Pthit[0] = phitl;
                              ptest.SimpleFit(hit_err);
	                        if (ptest._Chi2WithoutMS<chi2tmp) {
                                    chi2tmp = ptest._Chi2WithoutMS;
                          ...
                          if (_NoMoreTime()) { 
                              remove_track(ptrack); 
                              if (AMSEvent::debug) {
                                cout << " buildFalseTOFXPathIntegral Cpulimit Exceeded!!!! " << endl;
                              }
                              throw AMSTrTrackError(" Cpulimit Exceeded ");
                              return NTrackFound;
	                    }
...
integer AMSTrTrack::buildFalseTOFXPathIntegral_old(integer refit){
  {
    int nrh=0;
    for(int i=0;i<TKDBc::nlay();i++){
     nrh+= (AMSEvent::gethead()->getC("AMSTrRecHit",i))->getnelem();
...
///////////////////////////////////////////////////////////
integer AMSTrTrack::next_combination_FalseX(
                int index_min, int index_max, number par[2][3]){
         if (index_min<0) return 0;
         if (index_max>=TKDBc::patpoints(_Pattern)) return 0;
	   int index_run = index_min;
next_index:
         _Pthit[index_run] = _Pthit[index_run]->nextgood_FalseX_path(par);
         if (!_Pthit[index_run]){
	       if (index_run>=index_max) return 0;
             _Pthit[index_run] = AMSTrRecHit::firstgood_FalseX_path(_Pattern,index_run,par);
             index_run++;
	       goto next_index;
         }
...
///////////////////////////////////////////////////////////
AMSTrRecHit* AMSTrRecHit::firstgood_FalseX(integer pattern, integer index){
...
///////////////////////////////////////////////////////////
AMSTrRecHit* AMSTrRecHit::firstgood_FalseX_path(integer pattern, integer index,number par[2][3]){
...
///////////////////////////////////////////////////////////
AMSTrRecHit* AMSTrRecHit::nextgood_FalseX(){
...
///////////////////////////////////////////////////////////
AMSTrRecHit* AMSTrRecHit::nextgood_FalseX_path(number par[2][3]){
...
next_index:
         ...
         if (!_Pthit[index_run]){
	       if (index_run>=index_max) return 0;
             _Pthit[index_run] = AMSTrRecHit::firstgood_FalseTOFX_path(_Pattern,index_run,par);
             index_run++;
	       goto next_index;
         }
