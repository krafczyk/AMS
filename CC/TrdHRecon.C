...
int TrdHReconR::DoPrefit(vector<PeakXYZW> &seg_x, vector<PeakXYZW> &seg_y){
  ...
  for(int i=0;i!=nrh;i++){
    TRDHitRZD rzdi=TRDHitRZD((TrdRawHitR*)rhits[i]);
    for(int j=i+1;j!=nrh;j++){
      TRDHitRZD rzdj=TRDHitRZD((TrdRawHitR*)rhits[j]);
...
void TrdHReconR::ReadTRDEvent(vector<TrdRawHitR> r, vector<TrdHSegmentR> s, vector<TrdHTrackR> t){
  ...
  for(int i=0;i!=r.size();i++)trdhrecon.rhits.push_back(new TrdRawHitR(r[i]));
  for(int i=0;i!=s.size();i++)trdhrecon.hsegvec.push_back(new TrdHSegmentR(s[i]));  
  for(int i=0;i!=t.size();i++)trdhrecon.htrvec.push_back(new TrdHTrackR(t[i]));
}
