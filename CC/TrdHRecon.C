...
int TrdHReconR::clean_segvec(int debug){
  ...
  for(vector<TrdHSegmentR>::iterator s1=hsegvec.begin();s1!=hsegvec.end();s1++,c1++){
	...
	TRDHitRZD rzd1=TRDHitRZD(*s1->pTrdRawHit(hit1)); 
	for(int hit2=0;hit2<n2;hit2++){
	  TRDHitRZD rzd2=TRDHitRZD(*s2->pTrdRawHit(hit2)); 
...
void TrdHReconR::ReadTRDEvent(vector<TrdRawHitR> r, vector<TrdHSegmentR> s, vector<TrdHTrackR> t){
  ...
  for(int i=0;i!=r.size();i++)rhits.push_back(r[i]);
...
bool TrdHReconR::InitTDV(unsigned int bgtime, unsigned int edtime, int type,char * tempdirname){
  ...
  cout<<"tdv createdt"<<endl;

  ptdv->validate(begintime);
  
  cout<<"tdv validated"<<endl;
  if( ! readTDV(bgtime) ){
...
double TrdHReconR::GetTrdChargeMH(TrdHTrackR *trd_track, float beta, int z){
  TVector3 trd_dir, trd_pos;
  trd_dir.SetXYZ(trd_track->Dir[0],trd_track->Dir[1],trd_track->Dir[2]);
  trd_pos.SetXYZ(trd_track->Coo[0],trd_track->Coo[1],trd_track->Coo[2]);
