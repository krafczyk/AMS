...
AMSProducer::AMSProducer(int argc, char* argv[], int debug) throw(Error):_debug(debug){
if(_Head)throw Error("AMSProducer::AMSProducer-E-AMSProducerALreadyExists",0); 
else{
 char * ior=0;
 uinteger uid=0;
 for (char *pchar=0; argc>1 &&(pchar=argv[1],*pchar=='-'); (argv++,argc--)){
  ...
  try{
   ...
   if(CORBA::is_nil(obj))throw Error("AMSProducer::AMSProducer-F-CORBA::is_nil-1",2);
   DPS::Producer_var var=DPS::Producer::_narrow(obj);
   if(CORBA::is_nil(var))throw Error("AMSProducer::AMSProducer-F-CORBA::is_nil-2",2);
   ...
   if(_fbin){
     time_t tcur;
     time(&tcur);
     _fbin<<"sendId "<< ctime(&tcur)<<" "<<ior<<endl;
   }
  }
   catch (CORBA::MARSHAL a){
   throw  Error("AMSProducer::AMSProducer-F-CORBA::MARSHAL",2);
  }
  catch (CORBA::COMM_FAILURE a){
   throw  Error("AMSProducer::AMSProducer-F-CORBA::COMM_FAILURE",2);
  }
  catch (CORBA::SystemException & a){
   throw  Error("AMSProducer::AMSProducer-F-CORBA::SystemError",2);
  }
}
_Head=this;
}
...
if(!logdir){
 ...
 if(!_fbin)throw Error("AMSProducer::_openLogFile-F-UnableOpenLogFile",2);
}
...
void AMSProducer::_getpidhost(uinteger uid){
 ...
 char name[256];
 int len=255;
 if(gethostname(name,len))throw Error("AMSProducer::_getpidhost-F-UnableGetHostName",2);
