...
void AMSMyTrack::add_hit(TrRecHitR* phit) {
        if (NHits>=AMSMyTrackConst::MAXLAY) { 
            cout << "AMSMyTrack WARNING: Can not add more than ";
            cout << AMSMyTrackConst::MAXLAY << " hits; new hit ignored" << endl;
            return;
            ...
            for(int i=0;i<NHits;i++) {
                  if (phit->Layer < pHit[i]->Layer) {
                        ...
                        cout << "AMSMyTrack WARNING: Can not use two hits on the same layer;";
...
void AMSMyTrack::del_hit(TrRecHitR* phit) {
        for(int i=0;i<NHits;i++) {
            if (pHit[i]==phit) {
        ...
        cout << "AMSMyTrack WARNING: Hits not found on track;" << endl;
        cout << "                     delete operation ignored" << endl;
}

void AMSMyTrack::reset() {
        NHits = 0;
}

void AMSMyTrack::use_hits_from(TrTrackR* ptrack) {
        NHits = 0;
...
bool AMSMyTrack::Fit(){
  ...
  // Consistency check on the number of hits
    if (NHits<3 || NHits>AMSMyTrackConst::MAXLAY) return false;

  // Get hit positions and uncertainties
  // Scale errors (we will use sigmas in microns)
    float hits[AMSMyTrackConst::MAXLAY][3];
    float b[AMSMyTrackConst::MAXLAY][3];
    float sigma[AMSMyTrackConst::MAXLAY][3];
    for (int i=0;i<NHits;i++){
      for (int j=0;j<3;j++){
  ...
  // Lenghts
    float len[AMSMyTrackConst::MAXLAY];
    for (int i=0;i<NHits;i++){
      if (i==0){
        len[i] = 0.;
      }
  ...
  // Calculate path integrals
    double PathIntegral_x[AMSMyTrackConst::MAXLAY][3];
    double PathIntegral_u[AMSMyTrackConst::MAXLAY][3];
  ...
  // F and G matrices
    const int idim = 5;
    double d[2*AMSMyTrackConst::MAXLAY][idim];
    for (int i=0;i<NHits;i++) {
      int ix = i;
      int iy = i+NHits;
  ...
  // Get Chi2/Ndof
    if (! BFieldOn) {
        ...
        P0[2] = hits[0][2];
        return true;
    } else if (Param[4]!=0.0 && RootCovariance(4,4)>0.0) {
    //} else if (Param[4]!=0.0 && InvCov[4][4]>0.0) {
        Chi2 /= (2.*NHits-5.);
        Chi2StrLine /= (NHits-2.);
        Rigidity = 2.997E-4/Param[4];
        ErrRigidity = sqrt(RootCovariance(4,4))/2.997;
        //ErrRigidity = sqrt(InvCov[4][4])/2.997E-4;
        Theta = acos(-sqrt(1-Param[2]*Param[2]-Param[3]*Param[3]));
        Phi = atan2(Param[3],Param[2]);
        P0[0] = Param[0];
        P0[1] = Param[1];
