//  $Id: geant3.C,v 1.152.2.2 2012/05/12 07:14:32 barao Exp $
...
#pragma omp parallel  default(none),shared(cpulimit,std::cout,std::cerr,amsffkey_,selectffkey_,gcflag_,run,event,tt,oldtime,count,nchunk,ia,Waiting), private(pdaq), copyin(LIPC2F,badpix_ams,badpix_lip,badpmt_lip) 
{
...
//      AMSProducer::gethead()->sendEventTagEnd(ptdv->getname(),p->getrun(),insert,begin,end,first,last,p->getnelem(),fail);       
     ...
     AMSProducer::gethead()->getRunEventInfo(true);
     res=DAQEvent::init();
    }
