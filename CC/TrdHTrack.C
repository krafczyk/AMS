...
float TrdHTrackR::GetPathLengthMH(int layer, int ladder, int tube, int i){           // returns pathlength in tube if hit
    ...
    TVector3 tr_pos;tr_pos.SetXYZ(Coo[0],Coo[1],Coo[2]);
    TVector3 tr_dir;tr_dir.SetXYZ(Dir[0],Dir[1],Dir[2]);
    TVector3 pos0=tr_pos+(z-tr_pos.z())/tr_dir.z()*tr_dir;
    
    TVector3 cross[2];
    double W=0, p=0, q=0, x1=0, y1=0, z1=0, x2=0, y2=0, z2=0;
    
    if(dir==0){
      W= 1/(tan(tr_dir.Theta())*cos(tr_dir.Phi()));
      ...
      if(fabs((Pi/2)-fabs(tr_dir.Phi()))<0.000001) {
	x1= pos0.x();
	x2= pos0.x();
      }
      else{
    ...
    y1=pos0.y()+tan(tr_dir.Phi())*(x1-pos0.x());
    y2=pos0.y()+tan(tr_dir.Phi())*(x2-pos0.x());
    
    if(fabs(pos0.x()-(double)r)<(double)radius){
      if(((tr_dir.Phi()<Pi/2 && tr_dir.Phi()>-Pi/2 ) &&
	  tr_dir.Theta()>Pi/2) || ((tr_dir.Phi()>Pi/2 || tr_dir.Phi()<-Pi/2 )&&
				   tr_dir.Theta()<Pi/2) ){
	...
	z2=sqrt(((double)radius*(double)radius)-((x2-(double)r)*(x2-(double)r)))+(double)z;
    ...
    else if ((pos0.x()-(double)r > (double)radius && tr_dir.Theta()>Pi/2
	      && (tr_dir.Phi()>Pi/2 || tr_dir.Phi()<-Pi/2)) ||
	     (pos0.x()>(double)radius && tr_dir.Theta()<Pi/2 && (tr_dir.Phi()<Pi/2 && tr_dir.Phi()>-Pi/2)) ||
	     (pos0.x()-(double)r <0-(double)radius && tr_dir.Theta()<Pi/2 &&
	      (tr_dir.Phi()>Pi/2 || tr_dir.Phi()<-Pi/2)) || (pos0.x()<0-(double)radius && tr_dir.Theta()>Pi/2 &&
							     (tr_dir.Phi()<Pi/2 && tr_dir.Phi()>-Pi/2))){
      ...
      z2=sqrt(((double)radius*(double)radius)-((x2-(double)r)*(x2-(double)r)))+(double)z;
	       ...
	       tr_dir.Theta()>Pi/2 && (tr_dir.Phi()>Pi/2 || tr_dir.Phi()<-Pi/2)) ||
	      (pos0.x()<0-(double)radius && tr_dir.Theta()<Pi/2 && (tr_dir.Phi()<Pi/2 && tr_dir.Phi()>-Pi/2)) ||
	      (pos0.x()-(double)r >(double)radius && tr_dir.Theta()>Pi/2 &&
	       (tr_dir.Phi()<Pi/2 && tr_dir.Phi()>-Pi/2)) || (pos0.x()-(double)r >(double)radius &&
							     tr_dir.Theta()<Pi/2 && (tr_dir.Phi()>Pi/2 || tr_dir.Phi()<-Pi/2))){
    ...
    if((Pi-fabs(tr_dir.Phi()))<0.000001) {
      y1= pos0.y();
      y2= pos0.y();
    }
    else{
    ...
    x1=pos0.x()+(y1-pos0.y())/tan(tr_dir.Phi());
    x2=pos0.x()+(y2-pos0.y())/tan(tr_dir.Phi());
    
    if(fabs(pos0.y()-(double)r)<(double)radius){
      if((tr_dir.Phi()>0 && tr_dir.Theta()>Pi/2) || (tr_dir.Phi()<0 && tr_dir.Theta()<Pi/2) ){
    ...
    else if( (pos0.y()-(double)r >(double)radius && tr_dir.Theta()>Pi/2 &&
	      tr_dir.Phi()<0) || (pos0.y()-(double)r >(double)radius &&
				  tr_dir.Theta()<Pi/2 && tr_dir.Phi()>0) || (pos0.y()-(double)r<0-(double)radius &&
									     tr_dir.Theta()>Pi/2 &&tr_dir.Phi()>0) ||
	     (pos0.y()-(double)r<0-(double)radius && tr_dir.Theta()<Pi/2 && tr_dir.Phi()<0)){
      z1=sqrt(((double)radius*(double)radius)-((y1-(double)r)*(y1-(double)r)))+(double)z;
      z2=sqrt(((double)radius*(double)radius)-((y2-(double)r)*(y2-(double)r)))+(double)z;
    }
    else if( (pos0.y()-(double)r <0-(double)radius && tr_dir.Theta()>Pi/2
	      && tr_dir.Phi()<0) || (pos0.y()-(double)r <0-(double)radius &&
				     tr_dir.Theta()<Pi/2 &&  tr_dir.Phi()>0) || (pos0.y()-(double)r>(double)radius &&
										 tr_dir.Theta()>Pi/2 &&tr_dir.Phi()>0) ||
	     (pos0.y()-(double)r>(double)radius && tr_dir.Theta()<Pi/2 && tr_dir.Phi()<0) ){
      z1=0-sqrt(((double)radius*(double)radius)-((y1-(double)r)*(y1-(double)r)))+(double)z;
      z2=0-sqrt(((double)radius*(double)radius)-((y2-(double)r)*(y2-(double)r)))+(double)z;
    }
    //printf("1: %f %f %f 2: %f %f %f W: %f p: %f q: %f\n", x1, y1, z1, x2,y2, z2, W, p, q);
