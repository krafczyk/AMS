...
AMSID AMSCTCRawCluster::crgid(integer i){
    if(i==0)return AMSID("AGEL",100+_bar);
    else return AMSID("WLS ",200+_bar);
}
  void AMSCTCRawCluster::init(){
    integer geom=CTCDBc::getgeom();
    _pMap=new CTCMap[CTCDBc::getnblk()];
    assert(_pMap != NULL);
    if(geom == 0){
      // horizontal readout
      if(CTCDBc::getnwls() > CTCDBc::getnblk()){
        cerr << "AMSCTCRawCluster::init-F-Invalid nwls/nblk "<<CTCDBc::getnwls()<<" "<<
      CTCDBc::getnblk()<<endl;
      exit(1);
      }
    integer ia;
    for(ia=1;ia<CTCDBc::getnblk()+1;ia++){
     AMSCTCRawCluster d(0,ia,0);
...
void AMSCTCRawCluster::sictcdigi(){
  ...
  AMSCTCMCCluster * ptr=(AMSCTCMCCluster*)
  AMSEvent::gethead()->
   getheadC("AMSCTCMCCluster",0,1); // last 1  to test sorted container
   if(ptr ==0) return;
  geant * TmpA= (geant*)UPool.insert(sizeof(geant)*CTCDBc::getnblk());
  geant * TmpB= (geant*)UPool.insert(sizeof(geant)*CTCDBc::getnwls());
  VZERO(TmpA,sizeof(geant)*CTCDBc::getnblk()/4);
  VZERO(TmpB,sizeof(geant)*CTCDBc::getnwls()/4);
   ...
   while(ptr){
    ...
    if(z>0){
      number Response;
      if(det==0){
        AMSCTCRawCluster d(0,ptr->getbarno(),0);
        AMSgvolume *p= AMSJob::gethead()->getgeomvolume(d.crgid(0));
        assert(p != NULL);
        ...
        if(CTCDBc::getgeom()==0){
         ...
         #ifdef __AMSDEBUG__
          if (z1<-3.e-4)cerr <<"sictcdigi-E-z1 out of volume" <<z1<<" " <<p1<<endl;
          if (z2<-0.1)cerr <<"sictcdigi-E-z2 out of volume" <<z2<<" " <<p2<<endl;
         #endif
          if(z1<0)z1=0;         
          if(z2<0)z2=0;         
         if(fabs(z1-z2)>2.e-4)Response=z*CTCMCFFKEY.AbsLength[det]*(exp(-z2/CTCMCFFKEY.AbsLength[det])-
                                             exp(-z1/CTCMCFFKEY.AbsLength[det]))/(z1-z2);
         else  Response=z*exp(-fabs(z1)/CTCMCFFKEY.AbsLength[det]);
        }
        else{
         AMSPoint wls(-p->getpar(0),0,0);
         number z1=p1[0]-wls[0];
         number z2=p2[0]-wls[0];
          if (z1<-3.e-4)cerr <<"sictcdigi-E-z1 out of volume" <<z1<<" " <<p1<<endl;
          if (z2<-0.1)cerr <<"sictcdigi-E-z2 out of volume" <<z2<<" " <<p2<<endl;
          if(z1<0)z1=0;         
          if(z2<0)z2=0;         
         if(fabs(z1-z2)>2.e-4)Response=z*CTCMCFFKEY.AbsLength[det]*(exp(-z2/CTCMCFFKEY.AbsLength[det])-
                                             exp(-z1/CTCMCFFKEY.AbsLength[det]))/(z1-z2)/2;
         else  Response=z*exp(-fabs(z1)/CTCMCFFKEY.AbsLength[det])/2;
         AMSPoint wls1(p->getpar(0),0,0);
         z1=-p1[0]+wls1[0];
         z2=-p2[0]+wls1[0];
         #ifdef __AMSDEBUG__
          if (z1<-3.e-4)cerr <<"sictcdigi-E-z1 out of volume" <<z1<<" " <<p1<<endl;
    ...
    if( ptr->testlast()){
#ifdef __AMSDEBUG__
      if(det==0)assert( ptr->getbarno()>0 && ptr->getbarno()<CTCDBc::getnblk()+1);     
      else assert( ptr->getbarno()>0 && ptr->getbarno()<CTCDBc::getnwls()+1);     
#endif
      if(det ==0)TmpA[ptr->getbarno()-1]=value;
      else       TmpB[ptr->getbarno()-1]=value;
      value=0;
    }            
  ...
  for(ib=1;ib<CTCDBc::getnwls()+1;ib++){
     ...
     if(TmpB[ib-1]>0){
      ...
      new AMSCTCRawCluster(0,ib,ival));
     }      
  }
  UPool.udelete(TmpA); 
  UPool.udelete(TmpB); 
...
void AMSCTCRawCluster::_printEl(ostream & stream){
  stream <<"AMSCTCRawCluster  "<<_bar<<" "<<_signal<<endl;
}
...
int i;
if(AMSCTCCluster::Out( IOPA.WriteAll)){

if(init++==0){
//book the ntuple block
  HBNAME(IOPA.ntuple,"CTCClust",CTCCLN.getaddress(),
  "CTCCluster:I*4,CTCStatus:I*4, CTCCoo(3):R*4,CTCErCoo(3):R*4,CTCRawSignal:R*4,CTCSignal:R*4,CTCESignal:R*4");
...
void AMSCTCCluster::_printEl(ostream & stream){
  stream <<"AMSCTCCluster "<<_Status<<" "<<_Coo<<  " "<<_ErrorCoo<<" "<<
...
AMSContainer *p =AMSEvent::gethead()->getC("AMSTOFCluster",0);
 if(p)p->printC(cout);
}
...
for (int i=0;i<maxpl;i++){ 
 ...
 && xplane[i]>= xplane[i+1] ){
  ...
  if(edep>TOFRECFFKEY.ThrS){
    //    cout << cofg <<endl;
   AMSEvent::gethead()->addnext(AMSID("AMSCTCCluster",0),
   new     AMSCTCCluster(status,cofg,ecoo,edep,sqrt(edep)));
   xplane[i-1]=0;
   xplane[i]=0;
   xplane[i+1]=0; 
   i=0;
