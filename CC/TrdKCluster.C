...
double TrdKCluster::TRDTrack_PathLengthLikelihood_Curved(Double_t *par){
  int size= NHits();
  double result=0;
...
void TrdKCluster::KounineRefit_Curved(AMSPoint& P_fit, AMSDir& D_fit,
                               const AMSPoint& P_init,
                               const AMSDir& D_init) {
  Double_t dDD  = 0.04;
  Double_t dAA  = 0.004;
 ...
 refit:
  ...
  for (Int_t m=0; m<5; m++ ) {
    ...
    W[m] = TRDTrack_PathLengthLikelihood_Curved(par);
  }
...
void TrdKCluster::FitTRDTrack(int method, int hypothesis){
    ...
    else  if(method==6) FitTRDTrack_PathLength_KFit_Curved(hypothesis);
    else{
...
void TrdKCluster::FitTRDTrack_PathLength_KFit_Curved(int particle_hypothesis){
  ...
  KounineRefit_Curved(TRDP0,TRDDir,P0,Dir);
