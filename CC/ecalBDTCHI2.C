...
float EcalShowerR::GetEcalBDTCHI2(AMSEventR *pev, unsigned int iBDTCHI2VERSION)
{
   ...
   if (id_maxspan == -1)
   {
      if (BCHI2_DEBUG) std::cerr << " !!! Cannot find a good track associated to this shower! EcalBDTCHI2 will return -999.\n";
      return -999;
   }

   if (ecalchi2 == NULL) ecalchi2 = new EcalChi2();
   ecalchi2->process(trk_track, this, id_maxspan);
   ...
   for (unsigned int ilayer = 0; ilayer < nLAYERs; ++ilayer)
   {
      ...
      //get Chi2 per layer; lateral Chi2 defined only id ED>2 !!!
      if (EnergyD>2.) 
	{
	  LayerChi2[ilayer] = ecalchi2->get_chi2(ilayer);
	}
      else
	{
	  LayerChi2[ilayer] = 1.;
