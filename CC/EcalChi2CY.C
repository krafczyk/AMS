...
bool EcalPDF::init(char* fdatabase){
    ...
    hpdfele_Chi2plus_BDT=new TH2D("hpdfele_Chi2plus_BDT1","hpdfele_Chi2plus_BDT",256,-0.01,1.01,256,-1.01,1.01);
    hpdfpro_Chi2plus_BDT=new TH2D("hpdfpro_Chi2plus_BDT1","hpdfpro_Chi2plus_BDT",256,-0.01,1.01,256,-1.01,1.01);
    ...
    if(fdatabase==NULL){
        ...
        if(f==NULL){
            cout<<"Can not load f_F2Edep_Erg_RMS from "<<fdatabase<<endl;
            has_init=false;
        }
        else{
            for(int i1=0;i1<4;i1++)
...
float EcalPDF::get_mean(int flayer,float coo,float Erg,int type,double& grad){
    if(!has_init){
...
float EcalPDF::get_rms (int flayer,float coo,float Erg,int type,double& grad){
    if(!has_init){
...
float EcalPDF::get_prob(int flayer,float coo,float Erg,int type,double& grad){
    if(!has_init){
        cout<<"EcalPDF has not been loaded!"<<endl;
        return -1;
    }
...
float  EcalChi2::process(AMSEventR* ev, TrTrackR* trtrack, int iTrTrackPar){
    ...
    _erge=ev->pEcalShower(nmax)->EnergyE     ;
...
int EcalChi2::cal_chi2(int start_cell,int end_cell,int layer,double coo,float& chi2,float& chi22,float& chi23,float& chi24,float sign){
    ...
    */
    for(i1=start_cell;i1<=end_cell;i1++){
        ...
        cell_prob[i1]=ecalpdf->get_prob(layer,-1.*sign*(i1*SIZE+shiftxy[layer]-coo),_erg,0,cell_prob_grad[i1]);
        cell_probbar[i1]=ecalpdf->get_prob(layer,-1.*sign*(i1*SIZE+shiftxy[layer]-coo),_erg,1,cell_probbar_grad[i1]);
    ...
    for(i1=start_cell;i1<=end_cell;i1++){
        if(cell_rms[i1]<=0.0){
            cell_rms[i1]+=ecalpdf->get_rms(layer,-1.*sign*(i1*SIZE+shiftxy[layer]-coo-0.18),_erg,0,cell_rms_grad[i1]);
            cell_rms[i1]+=ecalpdf->get_rms(layer,-1.*sign*(i1*SIZE+shiftxy[layer]-coo-0.09),_erg,0,cell_rms_grad[i1]);
            cell_rms[i1]+=ecalpdf->get_rms(layer,-1.*sign*(i1*SIZE+shiftxy[layer]-coo+0.09),_erg,0,cell_rms_grad[i1]);
            cell_rms[i1]+=ecalpdf->get_rms(layer,-1.*sign*(i1*SIZE+shiftxy[layer]-coo+0.18),_erg,0,cell_rms_grad[i1]);
            cell_rms[i1]/=4.0;
            cell_rms_grad[i1]=0.;
    ...
    for(i1=start_cell;i1<=end_cell;i1++){
        ...
        if(Edep_raw[layer*72+i1]<=0){
            ...
            fg=cell_probbar_grad[i1];
            if(fg==fg){
                if(layer%4<2){
                    grad[1]+=fg;
                    grad[3]+=fg*(ecalz[layer]-ecalz[8]);
                }
                else{
                    grad[0]+=fg;
                    grad[2]+=fg*(ecalz[layer]-ecalz[8]);
                }
            }
        }
        else{
            delta=(cell_mean[i1]-Edep_raw[layer*72+i1]*scale)   ;
            f    =delta/(2.0*cell_rms[i1])                      ;
            chisq_chi2=delta*f                                  ;
            chisq_prob=-1.*log(cell_prob[i1])                   ;
            fg=2*f*cell_mean_grad[i1]-(chisq_chi2/cell_rms[i1])*cell_rms_grad[i1]-1./cell_prob[i1]*cell_prob_grad[i1];
            if(fg==fg){
                if(layer%4<2){
                    grad[1]+=fg;
                    grad[3]+=fg*(ecalz[layer]-ecalz[8]);
...
int   EcalAxis::process(AMSEventR* ev, int algorithm, TrTrackR* trtrack){
    ...
    if(trtrack!=NULL)
        ...
        //Try to find closed trtrack
        int    trki=-1	   ;
        double dist=1.0e6;
        for(int i2=0;i2<ev->nTrTrack();i2++){
            AMSPoint pnt1;
            AMSDir   dir1;
            TrTrackR* trtrack1= ev->pTrTrack(i2);
            trtrack1->Interpolate(ext_p0[2],pnt1,dir1);
            if(dist>sqrt(fabs((ext_p0[0]-pnt1[0])*(ext_p0[0]-pnt1[0])+(ext_p0[1]-pnt1[1])*(ext_p0[1]-pnt1[1])))){
                dist=sqrt(fabs((ext_p0[0]-pnt1[0])*(ext_p0[0]-pnt1[0])+(ext_p0[1]-pnt1[1])*(ext_p0[1]-pnt1[1])));
                trki=i2;
            }
        }
        if(dist<4)
            sign=ev->pTrTrack(trki)->GetRigidity()>0?1.:-1.;
    }
...
float EcalAxis::get_nchi2(AMSEventR* ev){
    ...
    _erge=ev->pEcalShower(maxi)->EnergyE;
    ecalchi2->_erge=_erge               ;
    process(ev,2);
    _chi2=ecalchi2->get_chi2();
    return ecalchi2->ecalpdf->normalize_chi2(_chi2,_erge);
}
