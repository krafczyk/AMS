...
public:
  ...
  AMSPoint(const number fx, const number fy,const number fz):_x(fx),_y(fy),_z(fz){};
  //! Explicit constructor
  AMSPoint(const number coo[3]): _x(coo[0]),_y(coo[1]),_z(coo[2]){};
  //! Explicit constructor
  AMSPoint(const geant coo[3]): _x(coo[0]),_y(coo[1]),_z(coo[2]){};
  //! Copy Constructor
  AMSPoint(const AMSPoint& o){_x=o._x;_y=o._y;_z=o._z;}
  //! it sets the position
  void setp(const number fx,const number fy,const number fz){_x=fx;_y=fy;_z=fz;}
  //! it sets the position
  void setp(const number fx[]){_x=fx[0];_y=fx[1];_z=fx[2];}
  //! it sets the position
  void setp(const geant fx[]){_x=fx[0];_y=fx[1];_z=fx[2];}
  ...
  AMSPoint mm3(const number [][3]);
  //! it does the matrix product (left) with a matrix represented by the input array
  AMSPoint mm3i(const number [][3]);
  //! it does the matrix product (left) with a matrix represented by the input array
  AMSPoint mm3i(const AMSRotMat&);
  ...
  AMSPoint abs() const {return AMSPoint(fabs(_x),fabs(_y),fabs(_z));}
  ...
  AMSPoint  operator/(const number& o) const {return AMSPoint(_x/o,_y/o,_z/o);}
  //! it returns an AMSPoint scaled by the factor 1/o 
  AMSPoint  operator/(const integer& o) const {return AMSPoint(_x/o,_y/o,_z/o);}
  //! it sets the current AMSPoint object to (o,o,o)
  AMSPoint&  operator=(const integer& o){_x=o;_y=o;_z=o;return *this;}
  //! it sets the current AMSPoint object to (o,o,o)
  AMSPoint&  operator=(const number& o){_x=o;_y=o;_z=o;return *this;}
  ...
  //! it returns the vector scalar product of the two AMSPoints
   number prod(const AMSPoint& o) const {return (_x*o._x+_y*o._y+_z*o._z);}
;
  ...
  number dist(const AMSPoint& o) const {return sqrt((_x-o._x)*(_x-o._x)+
	(_y-o._y)*(_y-o._y)+
	(_z-o._z)*(_z-o._z));}
  ...
  //! do some printout
  void Print() const {printf("point: %f  %f %f\n",_x,_y,_z);}
#ifdef __WRITEROOT__
...
class AMSDir :public AMSPoint{
  private:
    ...
    void _copy(const number x,const number y,const number z);
  public:
  ...
  AMSDir(const number theta, const number phi);
  ...
  AMSDir(const number fx, const number fy, const number fz){_copy(fx,fy,fz);}
  //! explicit constructor from an array
  AMSDir(const number fx[]){_copy(fx[0],fx[1],fx[2]);}
  //! it sets the direction from coo
  void setd(const number fx, const number fy, const number fz){_copy(fx,fy,fz);}
  //! it sets the direction from an array
  void setd(const number fx[]){_copy(fx[0],fx[1],fx[2]);}
  //! it does the vector cross product of two AMSDir
  AMSDir cross(const AMSDir& o) const;
  //! do some printout
  void Print() const {AMSPoint::Print();}
    ClassDef(AMSDir,1);
};
...
public:
  ...
  AMSRotMat(const number** nin);
  //! explicit constructor from an array
  AMSRotMat(const number nin[][3]);
  ~AMSRotMat(){}
  
  //! set the rotation matrix from an array pointer
  void SetMat(const number** nin);
  //! set the rotation matrix from an array 
  void SetMat(const number nin[][3]); 
  ...
  void SetRotAngles(const double alpha, const double beta, const double gamma);
  ...
  void GetRotAngles(double& alpha, double& beta, double& gamma) const;

  //! Invert matrix
  AMSRotMat& Invert();

  //! it returns the matrix element with the requested row,column
  number GetEl(const int row,const int col) const {return _nrm[row][col];}
