...
bool Clean_Event(AMSEventR *pev){
  ...
  int id_fs=track->iTrTrackPar(1,0,4);
  int id_up=track->iTrTrackPar(1,5,4);
  int id_low=track->iTrTrackPar(1,6,4);
  ...
  if(fabs(track->GetRigidity(id))>50.)  id=id_fs;
...
bool good_trd_event(AMSEventR *pev){
  ...
  if( trd_track->NTrdSegment()!=4 || trd_track->Chi2>4.) return false;
...
int trd_particle(AMSEventR *pev){
  ...
  if(pev->nParticle()==1){
    ...
    if(rig>50.){
      id=track->iTrTrackPar(1,0,4);
      rig=track->GetRigidity(id);    
    }
    ...
    if(! trdS->ProcessTrdEvt(pev, Debug) ) {
      ...
      // printf("mom: %f tof charge: %f like_ep: %f like_eh: %f like_ph: %f \n", pev->pParticle(0)->Momentum, tof_charge, eplik, eHelik, Heplik);
...
int ecal_particle(AMSEventR *pev){
  ...
  if(pev->nParticle()==1){
    ...
    if(tr_charge<1.5 && tof_charge>0 && tof_charge<1.5 &&  fabs(ecal_Ec/rig)>0.5 && fabs(ecal_Ec/rig)<100 && ecalbdt>0.142 && ecal_Ec>2.) return 0;
    else if(tr_charge<1.5 && tof_charge>0 && tof_charge<1.5 &&  fabs(ecal_Ec/rig)<0.25  && ecalbdt<-0.5) return 1; 
    else if(tr_charge>1.5 && tof_charge>1.5 &&  fabs(ecal_Ec/rig)<0.25  && ecalbdt<-0.5) return 2;
    else return -1;
  }
  return -1;
}
