#ifndef TEMPLATE_FITTER_2D_HH
#define TEMPLATE_FITTER_2D_HH
...
/**
 * @brief The TemplateFitter2D class
 *
...
public:
   ...
   * Only TH2F histos are supported right now.
   */
  void SetDataHistogram(TH2F* hist){ fInputData=hist; }

  /**
   * @brief Add Template Histogram
   * Only TH2F histos supported right now.
   ...
   */
  void AddTemplateHistogram(TH2F* hist);
  ...
  TCanvas* CreateResultDrawing(std::string canvasname, int width, int height, double chi2) const;
...
private:
  ...
  TH2F* fData;                           ///< Container for data histogram that is actually used in fit
  TH2F* fInputData;                      ///< Container for data histogram that is given to Fitter. Its range might be changed later.
  std::vector<TH2F*> fTemplates;         ///< Container for variable number of template distributions that are actually used in fit
  std::vector<TH2F*> fInputTemplates;    ///< Container for templates that are given to Fitter. Their range might be changed later.
  ...
  TH2F* fFitResult;                      ///< Histogram of fit result combined as sum from all templates
  std::vector<TH2F*> fResultHistos;      ///< Histograms of fit results of individual template distributions
