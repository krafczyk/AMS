...
bool ACsoft::ACQtProducer::ProduceECAL() {
  ...
  int nShowers = fAMSEvent->nEcalShower();
  for (int showerIndex = 0; showerIndex < nShowers; ++showerIndex) {
    ...
    ecalShower.fReconstructedEnergyElectron = pEcalShower->GetCorrectedEnergy(2 /* electron/positron */, 2 /* EnergyE */);
    ecalShower.fReconstructedEnergyPhoton = pEcalShower->GetCorrectedEnergy(1 /* photon */,              2 /* EnergyE */);
    ecalShower.fReconstructedEnergyStandaloneEstimator = pEcalShower->EnergyA; // LAPP ESE energy scale
    ...
    timer_ebdt5->Start(kFALSE);
    ecalShower.fEstimators.append(pEcalShower->GetEcalBDT(fAMSEvent, 5 /* default BDT version */));
    timer_ebdt5->Stop();

    timer_ese2->Start(kFALSE);
    ecalShower.fEstimators.append(pEcalShower->EcalStandaloneEstimatorV2());
    timer_ese2->Stop();

    timer_ese3->Start(kFALSE);
    ecalShower.fEstimators.append(pEcalShower->EcalStandaloneEstimatorV3());
    timer_ese3->Stop();

    timer_echi2->Start(kFALSE);
    ecalShower.fEstimators.append(pEcalShower->GetEcalBDTCHI2(fAMSEvent, 3 /* default BDT chi2 version */));
    timer_echi2->Stop();

    timer_ebdt4->Start(kFALSE);
    ecalShower.fEstimators.append(pEcalShower->GetEcalBDT(fAMSEvent, 4 /* default BDT version */));
    timer_ebdt4->Stop();
