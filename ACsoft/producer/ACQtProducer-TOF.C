...
bool ACsoft::ACQtProducer::ProduceTOFBetas() {
  ...
  for (int betaIndex = 0; betaIndex < nBetaHs; ++betaIndex) {
    ...
    tofBeta.fTrackerTrackIndex = pBetaH->iTrTrack(); // index of associated tracker track
    ...
    for (int ilay=0; ilay<4; ilay++){
      if( pBetaH->TestExistHL(ilay) ) {
        ...
        if(pBetaH->IsGoodQPathL(ilay)) tofBeta.fChargesNew.append( fabs(pBetaH->GetQL(ilay)));
        else                           tofBeta.fChargesNew.append(-fabs(pBetaH->GetQL(ilay))); // flag insufficient path-length
...
bool ACsoft::ACQtProducer::ProduceTOFClusters() {
  ...
  for (int clusterIndex = 0; clusterIndex < nClusterHs; ++clusterIndex) { 
    ...
    if ( pTofClusterH->IsOneLT(0) && pTofClusterH->IsOneLT(1) ) // single TDC on both sides 
      TOFCluster.fTimeError =  fabs(pTofClusterH->ETime);
    else
      TOFCluster.fTimeError = -fabs(pTofClusterH->ETime); // flag multi TDC with negative error
    TOFCluster.fEnergy    = pTofClusterH->GetEdep();
    TOFCluster.fCharge    = pTofClusterH->GetQSignal();
