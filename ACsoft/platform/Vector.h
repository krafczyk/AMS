...
namespace WTF {
    ...
    public:
        ...
        {
            Q_ASSERT(shouldReallocateBuffer(newCapacity));
            m_capacity = newCapacity;
            if (newCapacity > std::numeric_limits<size_t>::max() / sizeof(T))
                CRASH();
    ...
    protected:
        ...
        ~VectorBufferBase()
        {
            // FIXME: It would be nice to find a way to Q_ASSERT that m_buffer hasn't leaked here.
        }
    ...
    public:
        ...
        void allocateBuffer(size_t newCapacity)
        {
            // FIXME: This should Q_ASSERT(!m_buffer) to catch misuse/leaks.
    ...
    private:
        ...
        const T* inlineBuffer() const { return reinterpret_cast<const T*>(m_inlineBuffer.buffer); }
...
// Works around an assert in VS2010. See https://connect.microsoft.com/VisualStudio/feedback/details/558044/std-copy-should-not-check-dest-when-first-last
    ...
    Vector<T, inlineCapacity>& Vector<T, inlineCapacity>::operator=(const Vector<T, otherCapacity>& other)
    {
        ...
        Q_ASSERT(!typelessPointersAreEqual(&other, this));
