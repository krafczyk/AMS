...
for(int i=0;i<4;i++){
for(int k=0;k<14;k++){
if(ntuple->_LVL1.TOF[i] & (1<<k))((TH1F*)_filled2[i])->Fill(float(k+1),1.);
if(ntuple->_LVL1.TOF[i] & (1<<(16+k))){
  ((TH1F*)_filled2[i+4])->Fill(float(k+1),1.);
  nhit++;
}
}
}
...
if((ntuple->_TOF.Status[i] & 256)==0){
  for(int k=0;k<14;k++){
    ...
    _filled2[ntuple->_TOF.Plane[i]+7]->Fill(ntuple->_TOF.Bar[i],float(k+1),1.);
  }
