//  $Id: uShuttlePar.C,v 1.7 2001/01/22 17:32:54 choutko Exp $
#include <typedefs.h>
#include <cern.h>
#include <ctype.h>
#include <iostream.h>
...
PAWC_DEF PAWC;
 ...
 class ShuttlePar{
  public:
   ...
   float d2f(double d);
   integer Cas2Shuttle(CASblock & b1, ShuttlePar & b32, ShuttleNt & b32Nt);
...
void convert (int begin, int end,char file[]){
	   ...
	   time_t badt=0;
           if(begin==0){
              HBNT(1,"CAS Data"," ");
              HBNAME(1,"CASData",(int*)(&blockNt),"StationR,StationTheta,StationPhi,GrMedPhi,StationYaw,StationPitch,StationRoll,StationSpeed,SunR,SunTheta,SunPhi,StThetaGTOD,StPhiGTOD,VelTheta,VelPhi,VelThetaGTOD,VelPhiGTOD,Time:I");
           ...
           if(fin){
              ...
              while(!fin.eof()){
                ...
                int ret=Cas2Shuttle(buffer,block,blockNt);
                if(ret>0){
...
integer Cas2Shuttle(CASblock & buffer, ShuttlePar & block, ShuttleNt & blockNt){
...
uint para,statu;
	...
	return -1;
      ...
      block.Time=blockNt.Time=utime-ClockTune(utime);
}
